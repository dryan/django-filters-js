...
export function date(value: Date, format: "L"): boolean;
export function date(value: Date, format: DateFormat): string;
export function date(value: Date, format: undefined): string;
export function date(value: Date, format: Exclude<string, "L">): string;
export function date(
  value: Date,
  format: DateFormat | string = date.defaultFormats.date
): string | null | boolean {
  /*
  ...
  */
  ...
  const normalize12Hours = (hours: number) => {
    if (hours > 12) {
      hours = hours - 12;
    } else if (hours === 0) {
      hours = 12;
  ...
  const formats: { [key: string]: string | boolean } = {
    ...
    f: ((v) => {
      const ret: (string | number)[] = [normalize12Hours(v.getHours())];
      if (v.getMinutes() !== 0) {
        ret.push(":");
  ...
  formats.c = [
    ...
    formats.s,
    ".",
    padStart(formats.u as string, 6, 0),
  ].join("");
  // RFC 2822
